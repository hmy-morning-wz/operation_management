        ...
        <el-row class="lh40 mt10">
          <el-col :span="24">
            ...
            <div class="float-left mr10 w120">
              <el-input placeholder="请输入内容" v-model="searchParams.voucherName" clearable></el-input>
            </div>
      ...
      <div class="table">
        <div class="tableList">
          <el-table :data="tableData">
            ...
            <el-table-column prop="voucherName" label="优惠券名称" min-width="50" align="center"></el-table-column>
            <el-table-column prop="voucherTypeStr" label="类型" min-width="50" align="center"></el-table-column>
            <el-table-column prop="amount" label="面值" min-width="50" align="center"></el-table-column>
            ...
            <el-table-column label="条件" min-width="50" align="center">
              <template slot-scope="scope" style="text-align:center">
                <div v-if="scope.row.floorAmount">满{{scope.row.floorAmount}}元使用</div>
                <div v-else>无限制</div>
              </template>
            </el-table-column>
            <el-table-column prop="voucherQuantity" label="发行总量" min-width="50" align="center"></el-table-column>
            <el-table-column prop="sendCount" label="已领数量" min-width="50" align="center"></el-table-column>
            <el-table-column
              prop="statusStr"
              label="全部"
              min-width="80"
  ...
  methods: {
    renderHeaderOne(createElement) {
      ...
      return createElement("div", {
        domProps: {
          innerHTML: `<select type="text" class="select" id="select">
    ...
    // 编辑
    handleEditor(id) {
      const router = `./apliTemplateCoupon?id=${id}&type=${2}`;
      this.$router.push(router);
    },
    // 新建会议
    handleAdd() {
      const router = `./addCoupon?difftype=${3}`;
      this.$router.push(router);
    },
    // 查看详情
    handleConfigure(id) {
      const router = `./apliTemplateCoupon?id=${id}&type=${1}`;
      this.$router.push(router);
    },
    // 删除
    handleDel(id) {
